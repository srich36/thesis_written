...
The Finite Thrust Arc problem explored within this thesis was implemented using the 11 unkown parameters in equation \ref{particleUnkowns}
and corresponding objective function \ref{JwithPenalty}. Position and velocity bounds for the PSO algorithm are given by \ref{particleBounds}. Particle position updating was 
standard as follows
...
\noindent Where $i$ ranged from $1$ to the $11$ unknowns. \newline

\noindent Velocity updating was implemented as

\begin{equation}
    \label{velocityUpdating}
v_i = v_ic_i+c_c(pBest_i-p_i)+c_s(gBest-Pi)
\end{equation}

\noindent where the three accelerator coefficients were defined as

\begin{equation}
    \label{acceleratorCoefficients}
c_i = \dfrac{(1+rand(0,1))}{2} \quad c_c = 1.49445(rand(0,1)) \quad c_s = 1.49445(rand(0,1))
\end{equation}

\noindent and $rand(0,1)$ was a random number generated between 0 and 1. \newline
